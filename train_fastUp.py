...
# ------------------------ Utility ------------------------
def save_result(masked_tensor, pred_tensor, target_tensor, epoch, save_dir="results_twostage"):
...
# ------------------------ Training ------------------------
def train(masked_dir, original_dirs, epochs=30, batch_size=8, lr=2e-4, val_ratio=0.2):
    ...
    for epoch in range(epochs):
        ...
        for masked, original in tqdm(train_loader, desc=f"Epoch {epoch+1}"):
            ...
            coarse_output, refined_output = netG(masked)
        ...
        with torch.no_grad():
            for val_masked, val_original in val_loader:
                ...
                coarse_output, refined_output = netG(val_masked)
        ...
        print(f"[Epoch {epoch+1}] Train Loss G: {train_lossG/len(train_loader):.4f}, D: {train_lossD/len(train_loader):.4f} | Val Loss G: {val_lossG:.4f}")

        if val_lossG < best_val_loss:
            best_val_loss = val_lossG
